...
**Status:** FSLBM free-surface fully implemented and stable (commit `f1ac8de`, branch `moving-body`). Lagrangian bubble / kLa modules not yet started.
...
**Implementation status (2026-05-xx)**: Fully implemented and stable.
Commit `f1ac8de` on branch `moving-body`.
...
**Location**: `Source/LBM.cpp` line 5503
...
**Location**: `Source/LBM.cpp` line 5369
...
**Location**: `Source/LBM.cpp` line 5583
...
$$f^*_{\text{iv},q} = f^{eq}_{bq}(\rho_\text{iv}, \mathbf{u}_\text{iv}, T_\text{iv})
                    + f^{eq}_{q}(\rho_\text{iv}, \mathbf{u}_\text{iv}, T_\text{iv})
                    - f^\text{pre}_{\text{iv}, bq}$$

where $bq$ is the direction **outward** toward gas (opposite of $q$).

**Local density**: `ρ_iv = max(Σ_q f_pre[iv][q], 1.0)`.

Using `ρ=1` (Körner original) when `ρ_iv < 1` is safe.  But for `ρ_iv > 1`
(driven by impeller flow), `f_pre[bq] ≈ feq_bq(ρ_actual)` makes `f_star[q]`
negative — which compounds through collide.  Clamping `ρ_iv ≥ 1` prevents this.
...
Pressure tensor diagonal passed to `set_extended_equilibrium_value`:
...
|---|---|---|
| `Source/LBM.H` | — | `m_phi_fslbm`, `m_cell_type`, `m_free_surface`, `m_free_surface_z`; prototypes for `fslbm_advance_surface`, `fslbm_init_cell_type`, `fslbm_sync_isfluid_markers` |
| `Source/Constants.H` | — | `CELL_SOLID/GAS/INTERFACE/LIQUID`; `FSLBM_PHI_LO`; `FSLBM_PHI_HI` |
| `Source/LBM.cpp` | line 5369 | `LBM::fslbm_sync_isfluid_markers(lev)` |
| `Source/LBM.cpp` | line 5503 | `LBM::fslbm_init_cell_type(lev)` |
| `Source/LBM.cpp` | line 5583 | `LBM::fslbm_advance_surface(lev)` |
...
- [ ] Surface tension via Laplace pressure (if bubble shape matters)

---

### 11.12 Key Equations Reference

**ABB for f (Körner 2005 Eq. 7 / Schwarzmeier 2023 Eq. 11)** — Step 1b:

$$f^*_{\text{iv},q} = f^{eq}_{bq}(\rho_\text{iv}, \mathbf{u}_\text{iv}, T_\text{iv})
                    + f^{eq}_{q}(\rho_\text{iv}, \mathbf{u}_\text{iv}, T_\text{iv})
                    - f^\text{pre}_{\text{iv},bq}, \quad
\rho_\text{iv} = \max\!\left(\textstyle\sum_q f^\text{pre}_{\text{iv},q},\ 1\right)$$
...
- Schwarzmeier et al. (2023) JCP 473, 111753: FSLBM vs PFLBM benchmark
